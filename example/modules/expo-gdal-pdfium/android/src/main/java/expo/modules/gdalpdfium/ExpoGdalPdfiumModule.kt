...
class ExpoGdalPdfiumModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("processGeoPDF") { inputPath: String, outputPath: String, promise: Promise ->
      try {
        Log.i("ExpoGdalPdfium", "Processing GeoPDF: $inputPath -> $outputPath")
        ...
        if (!inputFile.exists()) {
          promise.resolve(
            createResponseMap(
              ...
              "FILE_NOT_FOUND",
              true,
              mapOf("errorDetails" to "File does not exist: $inputPath")
            )
          )
        ...
        if (!inputFile.canRead()) {
          promise.resolve(
            createResponseMap(
              ...
              true,
              mapOf("errorDetails" to "File cannot be read: $inputPath")
            )
          )
          return@AsyncFunction
        }
        ...
        if (dataset == null) {
          ...
          if (isGeodetic) {
            Log.i("ExpoGdalPdfium", "Fallback GEODETIC detected")

            var metadata = tryExtractMetadataWithOGR(inputPath)
            var metadataSource = "REGEX"
            ...
            val renderPayload = renderResult?.get("result") as? Map<String, Any> ?: emptyMap()
            ...
            promise.resolve(
              createResponseMap(
                ...
                false,
                mapOf(
                  "imagePath" to outputPath,
                  "width" to (renderPayload["width"] ?: ""),
                  "height" to (renderPayload["height"] ?: ""),
                  "renderedWith" to "PdfRenderer",
                  "metadata" to mapOf(
                    "metadataAvailable" to (metadata != null),
                    "metadataSource" to metadataSource,
                    "coordinatesAreWGS84" to (metadata != null),
                    "isGeodetic" to true,
                    "fallbackMetadataUsed" to true,
          ...
          promise.resolve(
            createResponseMap(
              ...
              "GDAL_OPEN_FAILED",
              true,
              mapOf("errorDetails" to (gdalError ?: "Unknown error"))
            )
          )
          return@AsyncFunction
        }
        ...
        try {
          ...
          val metadata = extractMetadata(dataset, inputPath)

          val renderResult = extractImage(dataset, outputPath)

          if (renderResult["success"] != true) {
            val renderErrorResult: Map<String, Any> = renderResult.mapValues { it.value as Any }
          ...
          val successResult: Map<String, Any> = mapOf(
            "imagePath" to (renderResult["imagePath"] ?: ""),
            "width" to (renderResult["width"] ?: ""),
            "height" to (renderResult["height"] ?: ""),
            "renderedWith" to (renderResult["renderedWith"] ?: ""),
            "metadata" to metadata as Any
          )

          promise.resolve(
            createResponseMap(
              "GeoPDF processed successfully",
              "SUCCESS",
              false,
              successResult
            )
        ...
        Log.e("ExpoGdalPdfium", "Error processing GeoPDF", e)

        promise.resolve(
          createResponseMap(
            "Error processing GeoPDF",
            "ERROR",
            true,
            mapOf("errorDetails" to (e.message ?: "Unknown error"))
          )
  ...
  private fun extractMetadata(
    ...
    val geoTransform = DoubleArray(6)
    dataset.GetGeoTransform(geoTransform)
    val geoTransformStrings = geoTransform.map { it.toString() }
    ...
    // Coordenadas nativas desde GeoTransform
    val topLeftX = geoTransform[0]
    val topLeftY = geoTransform[3]
    val pixelWidth = geoTransform[1]
    ...
    var coordinatesAreWGS84 = false

    if (projectionWkt != null && projectionWkt.isNotEmpty()) {
      try {
        Log.i("ExpoGdalPdfium", "Projection WKT for metadata extraction: ${projectionWkt.take(200)}...")
        ...
        coordinatesAreWGS84 = true
        Log.i("ExpoGdalPdfium", "Metadata coordinates transformed to WGS84 successfully")
      } catch (e: Exception) {
        Log.e("ExpoGdalPdfium", "Failed to transform metadata coordinates to WGS84", e)
        Log.w("ExpoGdalPdfium", "Using native CRS coordinates instead")
      }
    } else {
      Log.w("ExpoGdalPdfium", "No projection information available. Using native coordinates.")
    }
    ...
    return mapOf(
      ...
      "driver" to driverName,
      "projection" to projection,
      "geoTransform" to geoTransformStrings,
      "bands" to bandsInfo,
      "metadataAvailable" to true,
      "metadataSource" to "GDAL",
      "coordinatesAreWGS84" to coordinatesAreWGS84,
      "isGeodetic" to false,
      "fallbackMetadataUsed" to false,
      "topLeft" to mapOf(
        "x" to topLeftLng.toString(),
        "y" to topLeftLat.toString()
      ),
      "topRight" to mapOf(
        "x" to topRightLng.toString(),
        "y" to topRightLat.toString()
      ),
      "bottomLeft" to mapOf(
        "x" to bottomLeftLng.toString(),
        "y" to bottomLeftLat.toString()
      ),
      "bottomRight" to mapOf(
        "x" to bottomRightLng.toString(),
        "y" to bottomRightLat.toString()
      ),
      "center" to mapOf(
        "x" to centerLng.toString(),
        "y" to centerLat.toString()
      )
    )
  }
  ...
  private fun extractImage(
    ...
    Log.i("ExpoGdalPdfium", "Rendering image from dataset: ${width}x${height}, bands: $bandCount")

    val bitmapConfig: Bitmap.Config = when {
      bandCount >= 3 -> Bitmap.Config.ARGB_8888
      bandCount == 1 -> Bitmap.Config.ALPHA_8
      else -> Bitmap.Config.ARGB_8888
    }
    ...
    try {
      if (bandCount >= 3) {
        ...
        if (redBand == null || greenBand == null || blueBand == null) {
          return mapOf(
            "success" to false,
            "error" to "Could not read RGB bands from dataset"
          )
        }
        ...
        if (grayBand == null) {
          return mapOf(
            "success" to false,
            "error" to "Could not read grayscale band"
          )
        }
        ...
        return mapOf(
          "success" to false,
          "error" to "Unsupported band count: $bandCount"
        )
      }
      ...
      if (!outputFile.exists() || outputFile.length() == 0L) {
        return mapOf(
          "success" to false,
          "error" to "Failed to write PNG file"
        )
      }
